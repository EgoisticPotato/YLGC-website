...
function CountUp({ end, suffix = '', isVisible }) {
  ...
  useEffect(() => {
    if (isVisible) {
      ...
      const incrementTime = Math.max(duration / endValue, 10);
...
export default function Stats() {
  const [titleRef, titleVisible] = useOnScreen({ threshold: 0.3, once: true });
  const [statsRef, statsVisible] = useOnScreen({ threshold: 0.1, once: true });
  ...
  return (
    <section className="relative overflow-hidden py-20 bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className="max-w-7xl mx-auto px-4 w-full relative z-10">
        ...
        {/* Stats Grid */}
        <div ref={statsRef} className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 mb-12 max-w-5xl mx-auto">
